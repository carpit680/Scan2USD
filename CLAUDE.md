...
- `eval/scene_quality.py` + `tools/isaac/render_heldout.py` + `tuning/` — quality quantification and auto-tuning: `render-heldout` renders the packaged scene headlessly in Isaac at held-out COLMAP cameras (poses read from COLMAP TXT, **not** `transforms.json` — different frame) and scores PSNR/SSIM/LPIPS into `scene_quality.json`; `scan2usd tune` loops cheap splat-cleanup trials (re-clean from `environment_splat_raw.usd`, no retrain) and opt-in 3DGRUT retrain trials against that score (`workspace/tuning/trials.json`, promoted winner → `<config>_tuned.yaml`). `configs/bedroom_scene.yaml` is the most measured profile — its comments record what each setting cost or bought; there is no golden/max-quality profile yet. Baked artifacts carry a `transform_hash` in metadata; `PipelineOrchestrator._baked_ready` forces re-bakes when the COLMAP→USD transform changes after baking.
